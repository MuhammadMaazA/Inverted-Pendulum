...
class PIDController:
    ...
    def compute(self, measurement, dt):
        ...
        error = self.setpoint - measurement  # Fixed sign to be setpoint - measurement
...
def record_experiment(controller_type, use_noise=False, disturbance_magnitude=10.0, duration=20.0):
    ...
    Args:
        controller_type: 1=PID, 2=Pole Place, 3=Nonlinear
    ...
    # Modify joint properties for better physics simulation
    p.changeDynamics(cart_id, pole_joint_index, 
                    jointDamping=0.1)  # Set very low damping for better oscillation
    
    # Apply an initial impulse to start the natural oscillation
    pendulum_tip_pos = [0, 0, 0.5]  # Approximate position of the pendulum tip
    p.applyExternalForce(cart_id, pole_joint_index, [0.5, 0, 0], pendulum_tip_pos, p.LINK_FRAME)
    ...
    while sim_time < duration:
        ...
        # Apply the control force to the cart
        if abs(force) > 0.001:  # Only apply non-zero forces
            p.applyExternalForce(cart_id, -1, [force, 0, 0], [0, 0, 0], p.WORLD_FRAME)
...
def compare_controllers(use_noise=False, disturbance_magnitude=10.0):
    ...
    print("Testing Pole Placement controller...")
    pole_place_data = record_experiment(2, use_noise, disturbance_magnitude)
    print("Testing Nonlinear controller...")
    ...
    metrics_table = {
        'Controller': ['PID', 'Pole Placement', 'Nonlinear'],
        'Response Time (s)': [
...
def main():
    ...
    current_dir = os.path.dirname(os.path.abspath(__file__))
    p.setAdditionalSearchPath(current_dir)
    cart_id = p.loadURDF("iteration3.urdf", [0, 0, 0.1],
    ...
    # Modify dynamics settings for more realistic behavior
    # Very important - explicitly disable the default motor control for all joints!
    for j in range(p.getNumJoints(cart_id)):
        p.setJointMotorControl2(cart_id, j, 
    ...
    # Setup debug parameters - KEEPING THE EXACT SAME UI LAYOUT AS ORIGINAL
    ...
    pid_p_id = p.addUserDebugParameter("PID P Gain", 0, 50, Kp)
    pid_i_id = p.addUserDebugParameter("PID I Gain", 0, 5, Ki)
    pid_d_id = p.addUserDebugParameter("PID D Gain", 0, 20, Kd)
    
    # Disturbance parameter - slider control
    ...
    # Initialize with a small initial angle (around 5 degrees) to see natural oscillation
    initial_angle = 0.1  # in radians (approx 5.7 degrees)
    p.resetJointState(cart_id, pole_joint_index, initial_angle)

    # Set low friction for wheels to allow cart to move freely
    for wheel_idx in wheel_indices:
        p.changeDynamics(cart_id, wheel_idx, 
                    lateralFriction=0.1,
                    rollingFriction=0.002,
                    spinningFriction=0.001)
    
    # Specifically adjust the pole joint to make it more responsive
    # Override URDF values with correct pybullet parameters
    p.changeDynamics(cart_id, pole_joint_index, 
                    jointDamping=0.05,  # Very low damping for better oscillation
                    angularDamping=0.01)  # Use correct parameter name
    
    # Apply a small lateral force to the pendulum tip to start natural oscillation
    pendulum_tip_pos = [0, 0, 0.5]  # Approximate position of the pendulum tip
    p.applyExternalForce(cart_id, pole_joint_index, [0.5, 0, 0], pendulum_tip_pos, p.LINK_FRAME)
    ...
    while sim_time < max_sim_time:
        ...
        # Apply the control force to the cart base
        if abs(force) > 0.001:  # Only apply non-zero forces
            # Apply force to the base link in world frame
            p.applyExternalForce(
                cart_id, 
                -1,  # Base link (-1)
                [force, 0, 0], 
                [0, 0, 0], 
                p.WORLD_FRAME  # Use world frame
            )
        ...
        if len(timestamps) % 100 == 0:
            ...
            if len(timestamps) > 1:
                ...
                controller_names = {0: "Off", 1: "PID", 2: "Pole Placement", 3: "Nonlinear"}
        ...
        if abs(pole_angle) > 0.8 or abs(cart_pos) > 3.0:
            ...
            # Reset joint state to zero angle
            ...
            # Apply small random forces to both sides to ensure natural oscillation
            for _ in range(3):  # Apply multiple small impulses
                # Random force in each direction
                right_force = np.random.uniform(0.1, 0.3)
                left_force = np.random.uniform(0.1, 0.3)
                
                # Apply to pendulum tip in both directions sequentially
                pendulum_tip_pos = [0, 0, 0.5]
                p.applyExternalForce(cart_id, pole_joint_index, 
                                    [right_force, 0, 0], 
                                    pendulum_tip_pos, p.LINK_FRAME)
                p.stepSimulation()
                time.sleep(0.05)
    ...
    # After simulation ends, run analysis
    plt.ioff()
    plt.close('all')
    print("Simulation completed. Running analysis...")
    
    # Run air drag effects analysis
    analyze_air_drag_effects()
    
    # Run controller comparison analysis
    compare_controllers()
    
    # Clean up resources
    p.disconnect()
    print("All analyses completed. Results saved as PNG files.")
