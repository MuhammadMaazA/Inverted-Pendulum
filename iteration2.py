...
def record_experiment(controller_type, use_noise=False, disturbance_magnitude=10.0, duration=20.0):
    ...
    # Modify joint properties to make pendulum more responsive
    p.changeDynamics(cart_id, pole_joint_index, 
                    jointDamping=0.5)  # Reduce joint damping
    
    # Apply an initial impulse to start the natural oscillation
    p.applyExternalForce(cart_id, -1, [10.0, 0, 0], [0, 0, 0], p.WORLD_FRAME)
...
def main():
    ...
    p.changeDynamics(cart_id, pole_joint_index, 
                    jointDamping=0.5)  # Reduce damping
    ...
    while sim_time < max_sim_time:
        ...
        if abs(pole_angle) > 0.8 or abs(cart_pos) > 3.0:
            ...
            # Apply an initial impulse to restart oscillation
            p.applyExternalForce(cart_id, -1, [10.0, 0, 0], [0, 0, 0], p.WORLD_FRAME)
