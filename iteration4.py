...
def main():
    ...
    while sim_time < max_sim_time:
        ...
        if abs(pole_angle) > 0.8 or abs(cart_pos) > 3.0:
            ...
            # Apply a clean initial impulse for proper oscillation
            pendulum_tip_pos = [0, 0, 0.5]
            p.applyExternalForce(cart_id, pole_joint_index, [1.0, 0, 0], pendulum_tip_pos, p.LINK_FRAME)
            for _ in range(5):  # Several steps to allow physics to take effect
                p.stepSimulation()
                time.sleep(0.01)
